...
widgets.ChartLinear.prototype.process = function(model) {
  ...
  view.axes = this.buildAxes(model);

  view.data = {};
  view.data_translate = [this.margin.left, this.margin.top].join(', ');
  var height = this.geom.height - this.margin.bottom - this.margin.top;
  for (var type in view.axes) {
    if (type in model.data) {
      view.data[type] = this.scaleData(
          model.data[type], view.axes[type], view.axes['bottom'], height);
    }
  }
...
widgets.ChartLinear.prototype.buildAxes = function(model) {
  var axes = {};
  for (var type in model.axes) {
    if (type in model.data) {
      this.updateMinMax(model.data[type], model.axes[type]);
    }
    var method = 'buildAxis' + type.charAt(0).toUpperCase() + type.slice(1);
...
widgets.ChartLinear.prototype.buildAxisBottom = function(_axis) {
...
widgets.ChartLinear.prototype.scaleData = function(data, axis, x_axis, height) {
  var d = [], current;
  for (var i = 0; i < data.length; i++) {
    ...
    for (var j = 0; j < data[i].length; j++) {
      ...
      if (typeof current === 'number') {
        curve.push(
            Math.round(x_axis.step * j / x_axis.scale) +
            ',' + (height - Math.round((current - axis.min) / axis.scale))
        );
      }
    }
    d.push(curve.join(' '));
...
widgets.ChartLinear.prototype.updateMinMax = function(data, axis) {
  var current,
